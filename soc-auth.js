...
const ADMIN_EMAIL = "eakhter@brooklynsteamcenter.org";
const ADMIN_PASSWORD = "akhter44";
const CANONICAL_GOOGLE_REDIRECT_URI = "https://mk-git-main-tawhidurrahman13s-projects.vercel.app/api/auth/google/callback";
const LEGACY_ADMIN_EMAILS = ["admin@socbootcamp.local", "akhter44@socbootcamp.local"];
...
function normalizeEmail(email) {
  ...
  if ((normalized === ADMIN_USERNAME || normalized === "eakhter@brooklynsteamcenter.org") && ADMIN_EMAIL) {
    return ADMIN_EMAIL;
  }
  return normalized;
}
...
async function deleteSiteUserByEmail(email) {
  await supabaseRequest("site_users", {
    method: "DELETE",
    query: `email=eq.${encodeURIComponent(normalizeEmail(email))}`
  });
}

async function enforceSingleAdminAccount() {
  await supabaseRequest("site_users", {
    method: "PATCH",
    query: `role=eq.admin&email=neq.${encodeURIComponent(ADMIN_EMAIL)}`,
    body: { role: "student", updated_at: new Date().toISOString() }
  });

  for (const email of LEGACY_ADMIN_EMAILS) {
    await deleteSiteUserByEmail(email);
  }

  const admin = await findSiteUserByEmail(ADMIN_EMAIL);
  if (admin) {
    await updateSiteUser(admin.id, {
      email: ADMIN_EMAIL,
      display_name: "Admin",
      role: "admin",
