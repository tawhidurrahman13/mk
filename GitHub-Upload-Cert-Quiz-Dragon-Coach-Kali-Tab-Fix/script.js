  ...
  if (certCoachHintButton) {
    certCoachHintButton.addEventListener("click", () => {
      ...
      if (hasAnswered) {
        ...
        if (!question) {
          renderCertCoachResult(Math.round((score / activeQuiz.questions.length) * 100));
          return;
        }
  ...
  function explainCertificationChoice(question, answerIndex) {
    ...
    if (answerIndex === question.correct) {
      if (lowerQuestion.includes("not")) {
        return `${answer} is correct because the question asks for the exception or the option that does not fit.`;
      }
      if (lowerQuestion.includes("true") || lowerQuestion.includes("false")) {
        return `${answer} is correct because it matches the truth value of the statement.`;
      }
      return `${answer} directly matches the concept tested by this question.`;
    }

    if (lowerQuestion.includes("not")) {
      return `${answer} may relate to the topic, but it is not the exception the question is asking for.`;
    }

    return `${answer} does not match the question as closely as ${correctAnswer}.`;
  }
